...
public class ProximityGestureFeature: BaseFeature<ProximityGestureData> {
    ...
    override func extractData<T>(with timestamp: UInt64, data: Data, offset: Int) -> FeatureExtractDataResult<T> {
        ...
        let parsedData = ProximityGestureData(with: data, offset: offset)
