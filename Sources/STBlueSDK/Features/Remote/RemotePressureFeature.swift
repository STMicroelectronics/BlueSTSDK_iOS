...
public class RemotePressureFeature: BaseFeature<RemoteData<PressureData>> {
    ...
    override func extractData<T>(with timestamp: UInt64, data: Data, offset: Int) -> FeatureExtractDataResult<T> {
        ...
        let parsedData = RemoteData<PressureData>(remoteId: remoteData.remoteId,
                                                  data: PressureData(with: data, offset: remoteData.offset))
