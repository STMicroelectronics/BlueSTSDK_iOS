...
@interface SettingsTableViewController ()<W2STSDKConfigControlDelegate>
...
-(void)asyncReadWithRegisterName:(W2STSDKRegisterName_e)name target:(W2STSDKRegisterTarget_e)target {
    if (self.configControl != nil && name != W2STSDK_REGISTER_NAME_NONE && target != W2STSDK_REGISTER_TARGET_BOTH) {
        W2STSDKRegister *reg = [W2STSDKRegisterDefines lookUpWithRegisterName:name];
        W2STSDKCommand *cmd = [W2STSDKCommand commandWithRegister:reg target:target];
...
- (void)refreshAction {
    ...
    @try {
        [self asyncReadWithRegisterName:W2STSDK_REGISTER_NAME_BLE_LOC_NAME target:W2STSDK_REGISTER_TARGET_PERSISTENT];
        [self asyncReadWithRegisterName:W2STSDK_REGISTER_NAME_BLE_PUB_ADDR target:W2STSDK_REGISTER_TARGET_PERSISTENT];
        [self asyncReadWithRegisterName:W2STSDK_REGISTER_NAME_FW_VER target:W2STSDK_REGISTER_TARGET_PERSISTENT];
        
        [self asyncReadWithRegisterName:W2STSDK_REGISTER_NAME_LED_CONFIG target:W2STSDK_REGISTER_TARGET_PERSISTENT];
        [self asyncReadWithRegisterName:W2STSDK_REGISTER_NAME_PWR_MODE_CONFIG target:W2STSDK_REGISTER_TARGET_PERSISTENT];
    }
...
-(void) writeLocalName:(NSString *)name {
    if (self.configControl != nil) {
        W2STSDKRegister *reg = [W2STSDKRegisterDefines lookUpWithRegisterName:W2STSDK_REGISTER_NAME_BLE_LOC_NAME];
        size_t s = reg.size * 2;
        ...
        W2STSDKCommand* cmd = [W2STSDKCommand commandWithRegister:reg target:W2STSDK_REGISTER_TARGET_PERSISTENT data:data];
...
-(void) writePubblicAddress:(NSString *)pubblicaddress {
    if (self.configControl != nil) {
        W2STSDKRegister *reg = [W2STSDKRegisterDefines lookUpWithRegisterName:W2STSDK_REGISTER_NAME_BLE_PUB_ADDR];
        size_t s = reg.size * 2;
        unsigned char buffer[s];
...
-(void) writeLedConfiguration:(NSString *)conf {
    if (self.configControl != nil) {
        W2STSDKRegister *reg = [W2STSDKRegisterDefines lookUpWithRegisterName:W2STSDK_REGISTER_NAME_LED_CONFIG];
        size_t s = reg.size * 2;
        unsigned char buffer[s];
...
-(void) writeLowPowerMode:(NSString *)mode {
    if (self.configControl != nil) {
        W2STSDKRegister *reg = [W2STSDKRegisterDefines lookUpWithRegisterName:W2STSDK_REGISTER_NAME_PWR_MODE_CONFIG];
        size_t s = reg.size * 2;
        unsigned char buffer[s];
...
-(void) writeFWUpgrade:(NSString *)fwupgrade {
    if (self.configControl != nil) {
        W2STSDKRegister *reg = [W2STSDKRegisterDefines lookUpWithRegisterName:W2STSDK_REGISTER_NAME_DFU_REBOOT];
        size_t s = reg.size * 2;
        unsigned char buffer[s];
        ...
        NSData* data = [NSData dataWithBytes:buffer length:s];
        W2STSDKCommand* cmd = [W2STSDKCommand commandWithRegister:reg target:W2STSDK_REGISTER_TARGET_SESSION data:data];
...
-(void) configControl:(W2STSDKConfigControl *) configControl didRegisterReadResult:(W2STSDKCommand *)cmd error:(NSInteger)error {
    ...
    W2STSDKRegister *reg = cmd.registerField;
    NSData * payload = cmd.data;
    NSIndexPath *indexPath = nil;
    
    unsigned char buffer[payload.length];
    [payload getBytes:buffer length:payload.length];
    W2STSDKRegisterName_e regname = [W2STSDKRegisterDefines lookUpRegisterNameWithAddress:reg.address target:cmd.target];
    ...
    if (checksize && (error == 0 || error == 2)) {
        switch(regname)
        {
            case W2STSDK_REGISTER_NAME_BLE_LOC_NAME: //local name
                indexPath = [NSIndexPath indexPathForRow:0 inSection:0];
                text = [NSString stringWithUTF8String:(const char *)(&buffer[1])];
                break;
            case W2STSDK_REGISTER_NAME_BLE_PUB_ADDR: //address
                ...
                text = [NSString stringWithFormat:@"%02X:%02X:%02X:%02X:%02X:%02X",
                        ...
                        buffer[0]];
                break;
            case W2STSDK_REGISTER_NAME_FW_VER: //address
                indexPath = [NSIndexPath indexPathForRow:2 inSection:0];
                text = [NSString stringWithFormat:@"%d.%d", buffer[0], buffer[1]];
                break;
            case W2STSDK_REGISTER_NAME_LED_CONFIG: //led config
                ...
                switch(buffer[0])
            {
                ...
                break;
            case W2STSDK_REGISTER_NAME_PWR_MODE_CONFIG: //power mode
...
-(void) configControl:(W2STSDKConfigControl *) configControl didRegisterWriteResult:(W2STSDKCommand *)cmd error:(NSInteger)error {
    if (error == 0 && cmd != nil && cmd.registerField != nil) {
        W2STSDKCommand *readCmd = [W2STSDKCommand commandWithRegister:cmd.registerField target:cmd.target];
        dispatch_async(dispatch_get_main_queue(),^{
